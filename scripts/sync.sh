...
# synching chromium/chromium
_dir="gits/chromium/chromium"
echo -e "\n# synching chromium/chromium"
[ ! -e $_dir ] || [ $(($(date +%s) - $(stat -c %Y "$_dir"))) -lt 3600 ] || ( git -C $_dir fetch --depth 1 && git -C $_dir reset --hard origin && touch $_dir )
[ -e $_dir ] || git clone --depth 1 git@github.com:chromium/chromium.git $_dir
mkdir -p chromium/
test -e gits/chromium/chromium/./
rsync -avm  --delete --delete-excluded  --include '*/' --include '*.proto' --include 'README.md' --exclude="*" gits/chromium/chromium/./ chromium/
test -z "$(find chromium/ -maxdepth 0 -empty)"
fi

if false; then

# synching apple/coremltools
_dir="gits/apple/coremltools"
echo -e "\n# synching apple/coremltools"
[ ! -e $_dir ] || [ $(($(date +%s) - $(stat -c %Y "$_dir"))) -lt 3600 ] || ( git -C $_dir fetch --depth 1 && git -C $_dir reset --hard origin && touch $_dir )
[ -e $_dir ] || git clone --depth 1 git@github.com:apple/coremltools.git $_dir
mkdir -p coremltools/
test -e gits/apple/coremltools/mlmodel/format/
rsync -avm  --delete --delete-excluded  --include '*/' --include '*.proto' --include 'README.md' --exclude="*" gits/apple/coremltools/mlmodel/format/ coremltools/
test -z "$(find coremltools/ -maxdepth 0 -empty)"
fi
git add .
