...
[ -e gits/dexidp/dex ] || git clone --depth 10 git@github.com:dexidp/dex.git gits/dexidp/dex
mkdir -p dex/api/
...
[ -e gits/googleapis/api-common-protos ] || git clone --depth 10 git@github.com:googleapis/api-common-protos.git gits/googleapis/api-common-protos
mkdir -p google/
...
[ -e gits/googleapis/googleapis ] || git clone --depth 10 git@github.com:googleapis/googleapis.git gits/googleapis/googleapis
...
[ -e gits/temporalio/api ] || git clone --depth 10 git@github.com:temporalio/api.git gits/temporalio/api
...
[ -e gits/uber/cadence-idl ] || git clone --depth 10 git@github.com:uber/cadence-idl.git gits/uber/cadence-idl
mkdir -p uber/
...
[ -e gits/yandex-cloud/cloudapi ] || git clone --depth 10 git@github.com:yandex-cloud/cloudapi.git gits/yandex-cloud/cloudapi
mkdir -p yandex/
...
[ -e gits/grpc/grpc ] || git clone --depth 10 git@github.com:grpc/grpc.git gits/grpc/grpc
...
[ -e gits/cosmos/cosmos-sdk ] || git clone --depth 10 git@github.com:cosmos/cosmos-sdk.git gits/cosmos/cosmos-sdk
mkdir -p cosmos/
...
[ -e gits/chrislusf/seaweedfs ] || git clone --depth 10 git@github.com:chrislusf/seaweedfs.git gits/chrislusf/seaweedfs
mkdir -p seaweedfs/
...
[ -e gits/envoyproxy/data-plane-api ] || git clone --depth 10 git@github.com:envoyproxy/data-plane-api.git gits/envoyproxy/data-plane-api
mkdir -p envoy/
...
[ ! -e gits/containerd/containerd ] || git -C gits/containerd/containerd pull
[ -e gits/containerd/containerd ] || git clone --depth 10 git@github.com:containerd/containerd.git gits/containerd/containerd
mkdir -p containerd/
...
[ ! -e gits/open-telemetry/opentelemetry-proto ] || git -C gits/open-telemetry/opentelemetry-proto pull
[ -e gits/open-telemetry/opentelemetry-proto ] || git clone --depth 10 git@github.com:open-telemetry/opentelemetry-proto.git gits/open-telemetry/opentelemetry-proto
...
[ ! -e gits/cockroachdb/cockroach ] || git -C gits/cockroachdb/cockroach pull
[ -e gits/cockroachdb/cockroach ] || git clone --depth 10 git@github.com:cockroachdb/cockroach.git gits/cockroachdb/cockroach
...
[ ! -e gits/esphome/esphome ] || git -C gits/esphome/esphome pull
[ -e gits/esphome/esphome ] || git clone --depth 10 git@github.com:esphome/esphome.git gits/esphome/esphome
mkdir -p esphome/
...
[ ! -e gits/dgraph-io/dgo ] || git -C gits/dgraph-io/dgo pull
[ -e gits/dgraph-io/dgo ] || git clone --depth 10 git@github.com:dgraph-io/dgo.git gits/dgraph-io/dgo
mkdir -p dgraph/api/
...
[ ! -e gits/etcd-io/etcd ] || git -C gits/etcd-io/etcd pull
[ -e gits/etcd-io/etcd ] || git clone --depth 10 git@github.com:etcd-io/etcd.git gits/etcd-io/etcd
mkdir -p etcd/
...
[ ! -e gits/kubernetes/cri-api ] || git -C gits/kubernetes/cri-api pull
[ -e gits/kubernetes/cri-api ] || git clone --depth 10 git@github.com:kubernetes/cri-api.git gits/kubernetes/cri-api
...
[ ! -e gits/envoyproxy/protoc-gen-validate ] || git -C gits/envoyproxy/protoc-gen-validate pull
[ -e gits/envoyproxy/protoc-gen-validate ] || git clone --depth 10 git@github.com:envoyproxy/protoc-gen-validate.git gits/envoyproxy/protoc-gen-validate
mkdir -p validate/
...
[ ! -e gits/authzed/spicedb ] || git -C gits/authzed/spicedb pull
[ -e gits/authzed/spicedb ] || git clone --depth 10 git@github.com:authzed/spicedb.git gits/authzed/spicedb
mkdir -p spicedb/
...
[ ! -e gits/markphelps/flipt ] || git -C gits/markphelps/flipt pull
[ -e gits/markphelps/flipt ] || git clone --depth 10 git@github.com:markphelps/flipt.git gits/markphelps/flipt
mkdir -p flipt/
...
[ ! -e gits/teamgram/teamgram-server ] || git -C gits/teamgram/teamgram-server pull
[ -e gits/teamgram/teamgram-server ] || git clone --depth 10 git@github.com:teamgram/teamgram-server.git gits/teamgram/teamgram-server
...
[ ! -e gits/hashicorp/consul ] || git -C gits/hashicorp/consul pull
[ -e gits/hashicorp/consul ] || git clone --depth 10 git@github.com:hashicorp/consul.git gits/hashicorp/consul
